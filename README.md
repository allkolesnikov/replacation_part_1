...
### 1 Скриншот 
На скриншоте показан статус службы MySQL на сервере Master. Служба успешно запущена для работы репликации.

![Replication](Images/status_mysql_2.png)

### 2 Скриншот
На скриншоте показан статус службы MySQL на сервере Slave. Служба находится в активном состоянии и готова к работе.
...
### 4 Скриншот
Показано состояние репликации на Slave-сервере. Параметры Replica_IO_Running и Replica_SQL_Running имеют значение Yes, что подтверждает успешную работу репликации.

![Replication](Images/replica.png)

### 5 Скриншот
Показан результат проверки репликации. Данные, созданные на Master-сервере, успешно переданы

![Replication](Images/Replication_test.png)
